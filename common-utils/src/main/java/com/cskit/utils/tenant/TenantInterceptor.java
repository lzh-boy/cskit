package com.cskit.utils.tenant;

import com.cskit.utils.tokens.SysAccountPO;
import com.cskit.utils.tokens.TokenUtil;
            ...
            if (invocation.getTarget() instanceof StatementHandler) {
                ...
                if (mappedStatement.getSqlCommandType().name().equals(COMMAND_TYPE) && !checkExistsTenant())
                    if (Objects.equals(getAnnotation(mappedStatement.getId()), null) && checkEnabled()) {
                        ...
                        try {
                            SYS_ACCOUNT_PO = TokenUtil.getStageUser((HttpServletRequest) SERVLET_REQUEST_ATTRIBUTES.getRequest());
                        } catch (Exception ex) {
